...
class StorageUtil {
    ...
    static File getApkFileDir() {
        return AppUtils.sApp.getExternalFilesDir(DIR_NAME_APK);
    }
    ...
    static File getApkFile(String version) {
        String appName;
        try {
            appName = AppUtils.sApp.getPackageManager().getPackageInfo(AppUtils.sApp.getPackageName(), 0).applicationInfo.loadLabel(AppUtils.sApp.getPackageManager()).toString();
        } catch (Exception e) {
