...
public class BGAUpgradeUtil {
    ...
    public static void installApk(File apkFile) {
        ...
        if (Build.VERSION.SDK_INT > Build.VERSION_CODES.M) {
            installApkIntent.setDataAndType(FileProvider.getUriForFile(AppUtils.sApp, getFileProviderAuthority(), apkFile), MIME_TYPE_APK);
            installApkIntent.addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION);
        } else {
            installApkIntent.setDataAndType(Uri.fromFile(apkFile), MIME_TYPE_APK);
        }

        if (AppUtils.sApp.getPackageManager().queryIntentActivities(installApkIntent, 0).size() > 0) {
            AppUtils.sApp.startActivity(installApkIntent);
        }
    }
    ...
    private static String getFileProviderAuthority() {
        try {
            for (ProviderInfo provider : AppUtils.sApp.getPackageManager().getPackageInfo(AppUtils.sApp.getPackageName(), PackageManager.GET_PROVIDERS).providers) {
